...
for ii = 1:length(RPMS)
    [IgvScaled, RotorScaled, StatorScaled] = JDT_SCALE(Solidities, MIT_RTIP, RE, PHI, RPMS(ii), Data, Igv, Rotor, Stator);
    JDT_PLOT_BLADE(IgvScaled, RotorScaled, StatorScaled, Sections);
    JDT_WRITE_BLADE_COORDINATES(IgvScaled, RotorScaled, StatorScaled, Sections, 2, RPMS(ii));
end
